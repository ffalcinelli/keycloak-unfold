...
test('fails when package.json or pom.xml is missing', async () => {
  ...
  try {
    ...
    assert.ok(
      error.stderr.includes('Error: Could not locate package.json or pom.xml.'),
      'Should print correct error message'
    );
  } finally {
...
test('fails when pom.xml has invalid version format', async () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'version-sync-test-'));
  try {
    const scriptDir = path.join(tempDir, 'scripts');
    fs.mkdirSync(scriptDir);
    ...
    // Create invalid pom.xml (missing <version>)
    fs.writeFileSync(path.join(tempDir, 'pom.xml'), '<?xml version="1.0"?><project></project>');
    ...
    assert.strictEqual(error.code, 1, 'Should have exit code 1');
    assert.ok(
      error.stderr.includes('Error: Could not find version in pom.xml'),
      'Should print correct error message'
    );
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
